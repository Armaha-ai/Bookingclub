...
const DEFAULT_COLLECTIONS: Collection[] = [
  {
    ...
    rating: 4.75,
    isPublic: true,
    recommendations: 'Эти книги формируют понимание русской культуры и литературы.',
  },
  {
    ...
    rating: 4,
    isPublic: true,
    recommendations: 'Книги, которые отражают современные проблемы.',
  },
];
...
export function Collections() {
  ...
  const handleCreateCollection = () => {
    ...
    setForm({ name: '', description: '', tags: '', recommendations: '', isPublic: true });
  ...
  const handleAddBook = () => {
    ...
    const updated = collections.map((c) => {
      if (c.id === selectedCollection.id) {
        const updatedCollection = { ...c, books: [...c.books, newBook] };
  ...
  const handleRemoveBook = (bookId: string) => {
    ...
    const updated = collections.map((c) => {
      if (c.id === selectedCollection.id) {
        const updatedCollection = { ...c, books: c.books.filter((b) => b.id !== bookId) };
        updatedCollection.rating = getRating(updatedCollection.books);
        setSelectedCollection(updatedCollection);
        return updatedCollection;
      }
  ...
  return (
    <section id="collections" ref={sectionRef} className="section-padding relative overflow-hidden bg-[#FAF3E0]">
                  ...
                  {collection.tags.length > 0 && (
                    <div className="flex gap-1 flex-wrap mb-3">
                      {collection.tags.slice(0, 2).map((tag) => (
                        <span key={tag} className="text-[9px] bg-[#E8C4B8]/30 text-[#C17A5A] px-2 py-0.5 rounded-full">{tag}</span>
                      ))}
                      {collection.tags.length > 2 && (
                        <span className="text-[9px] bg-[#E8C4B8]/30 text-[#C17A5A] px-2 py-0.5 rounded-full">+{collection.tags.length - 2}</span>
                      )}
                    </div>
                  )}
      ...
      {selectedCollection && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setSelectedCollection(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-8">
              <div className="flex items-start justify-between mb-6">
                ...
                <button onClick={() => setSelectedCollection(null)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[#E8C4B8]/50 transition-colors flex-shrink-0">
                ...
                <div className="mb-6">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {selectedCollection.tags.map((tag) => (
                      <span key={tag} className="text-xs bg-[#E8C4B8]/40 text-[#C17A5A] px-3 py-1 rounded-full">{tag}</span>
                    ))}
                  </div>
                </div>
              )}
              <div className="mb-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-sm text-[#3D2B1F]/60">
                    ...
                    {selectedCollection.rating > 0 && <> • <Star className="w-3 h-3 inline text-[#C17A5A]" /> {selectedCollection.rating}</>}
                  </div>
                </div>
                {selectedCollection.books.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {selectedCollection.books.map((book) => (
                      <div key={book.id} className="flex items-start justify-between p-3 bg-[#FAF3E0] rounded-lg border border-[#E8C4B8]/30 group hover:border-[#C17A5A]/50 transition-colors">
                        <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 flex-shrink-0">
                          <div className="flex gap-0.5">
                            {[...Array(5)].map((_, i) => (
                              <Star key={i} className={`w-3 h-3 ${i < book.rating ? 'text-[#C17A5A]' : 'text-[#E8C4B8]'}`} fill={i < book.rating ? 'currentColor' : 'none'} />
                            ))}
                          </div>
                          {selectedCollection.userAdded && (
                            <button onClick={() => handleRemoveBook(book.id)} className="opacity-0 group-hover:opacity-100 text-[#C17A5A] hover:text-[#8B5A3C] transition-all">
                ...
                {selectedCollection.userAdded && (
                  <button onClick={() => setShowAddBookModal(true)} className="mt-3 w-full px-4 py-2 text-sm text-[#A8C5A0] border border-[#A8C5A0]/50 rounded-lg hover:bg-[#A8C5A0]/10 transition-colors flex items-center justify-center gap-2">
                    <Plus className="w-4 h-4" />
                    Добавить книгу
                  </button>
                )}
              ...
              <div className="flex gap-3 pt-6 border-t border-[#E8C4B8]/30">
                {selectedCollection.isPublic && (
                  <button onClick={() => handleCopyLink(selectedCollection.id)} className="px-4 py-2 text-sm text-[#3D2B1F]/70 hover:bg-[#E8C4B8]/20 rounded-lg transition-colors flex items-center gap-2">
                    <Copy className="w-4 h-4" />
                    {copiedId === selectedCollection.id ? 'Скопировано!' : 'Копировать ссылку'}
                  </button>
                )}
                {selectedCollection.userAdded && (
                  <button onClick={() => handleDeleteCollection(selectedCollection.id)} className="px-4 py-2 text-sm text-[#C17A5A] hover:bg-[#C17A5A]/10 rounded-lg transition-colors flex items-center gap-2">
                    <Trash2 className="w-4 h-4" />
                    Удалить подборку
                  </button>
                )}
                <button onClick={() => setSelectedCollection(null)} className="ml-auto px-6 py-2 bg-[#A8C5A0] text-white rounded-lg hover:bg-[#8aab82] transition-colors font-serif text-sm">
                  Закрыть
                </button>
              </div>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder="Например: Лучшие детективы" className="w-full px-4 py-2.5 bg-[#FAF3E0] border border-[#E8C4B8]/50 rounded-xl text-sm text-[#3D2B1F] placeholder-[#3D2B1F]/40 focus:outline-none focus:border-[#C17A5A]" />
                </div>
                <div>
                  <label className="block text-sm text-[#3D2B1F]/80 mb-1">Описание *</label>
                  <textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} rows={2} placeholder="О чем эта подборка?" className="w-full px-4 py-2.5 bg-[#FAF3E0] border border-[#E8C4B8]/50 rounded-xl text-sm text-[#3D2B1F] placeholder-[#3D2B1F]/40 focus:outline-none focus:border-[#C17A5A] resize-none" />
                </div>
                <div>
                  <label className="block text-sm text-[#3D2B1F]/80 mb-1">Почему эта подборка? (опционально)</label>
                  <textarea value={form.recommendations} onChange={(e) => setForm((p) => ({ ...p, recommendations: e.target.value }))} rows={2} placeholder="Объясните читателям, почему она интересна..." className="w-full px-4 py-2.5 bg-[#FAF3E0] border border-[#E8C4B8]/50 rounded-xl text-sm text-[#3D2B1F] placeholder-[#3D2B1F]/40 focus:outline-none focus:border-[#C17A5A] resize-none" />
                </div>
                <div>
                  <label className="block text-sm text-[#3D2B1F]/80 mb-1">Теги (через запятую)</label>
                  <input type="text" value={form.tags} onChange={(e) => setForm((p) => ({ ...p, tags: e.target.value }))} placeholder="детектив, приключение, мистика" className="w-full px-4 py-2.5 bg-[#FAF3E0] border border-[#E8C4B8]/50 rounded-xl text-sm text-[#3D2B1F] placeholder-[#3D2B1F]/40 focus:outline-none focus:border-[#C17A5A]" />
                </div>
                <div className="flex items-center gap-3">
                  <input type="checkbox" id="public-toggle" checked={form.isPublic} onChange={(e) => setForm((p) => ({ ...p, isPublic: e.target.checked }))} className="w-4 h-4 rounded border-[#E8C4B8] accent-[#A8C5A0]" />
                  <label htmlFor="public-toggle" className="text-sm text-[#3D2B1F]/80">Сделать подборку общедоступной</label>
                </div>
                <button onClick={handleCreateCollection} disabled={!form.name || !form.description} className="w-full btn-primary flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed mt-6">
                  <Plus className="w-4 h-4" />
                  Создать подборку
                </button>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => setShowAddBookModal(false)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[#E8C4B8]/50 transition-colors">
                  <X className="w-4 h-4 text-[#3D2B1F]" />
                </button>
              </div>
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={bookForm.title} onChange={(e) => setBookForm((p) => ({ ...p, title: e.target.value }))} placeholder="Например: Война и мир" className="w-full px-4 py-2.5 bg-[#FAF3E0] border border-[#E8C4B8]/50 rounded-xl text-sm text-[#3D2B1F] placeholder-[#3D2B1F]/40 focus:outline-none focus:border-[#C17A5A]" />
                </div>
                <div>
                  <label className="block text-sm text-[#3D2B1F]/80 mb-1">Автор *</label>
                  <input type="text" value={bookForm.author} onChange={(e) => setBookForm((p) => ({ ...p, author: e.target.value }))} placeholder="Например: Лев Толстой" className="w-full px-4 py-2.5 bg-[#FAF3E0] border border-[#E8C4B8]/50 rounded-xl text-sm text-[#3D2B1F] placeholder-[#3D2B1F]/40 focus:outline-none focus:border-[#C17A5A]" />
                </div>
                <div>
                  <label className="block text-sm text-[#3D2B1F]/80 mb-2">Оценка</label>
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button key={star} onClick={() => setBookForm((p) => ({ ...p, rating: star }))} className="transition-colors">
                        <Star className={`w-6 h-6 ${star <= bookForm.rating ? 'text-[#C17A5A]' : 'text-[#E8C4B8]'}`} fill={star <= bookForm.rating ? 'currentColor' : 'none'} />
                      </button>
                    ))}
                  </div>
                </div>
                <button onClick={handleAddBook} disabled={!bookForm.title || !bookForm.author} className="w-full btn-primary flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed mt-6">
                  <Plus className="w-4 h-4" />
                  Добавить книгу
                </button>
              </div>
